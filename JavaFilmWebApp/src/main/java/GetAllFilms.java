...
throws ServletException, IOException {
	...
	if ("json".equals(format)) {
	    ...
	    RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/JSP/AllFilms/AllFilmsJSON.jsp");
	    dispatcher.forward(request, response);
	    }
		
	if ("xml".equals(format)) {
		response.setContentType("text/xml");
	    RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/JSP/AllFilms/AllFilmsXML.jsp");
	    dispatcher.forward(request, response);
	}
	if ("string".equals(format)) {
		response.setContentType("text/plain");
	    RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/JSP/AllFilms/AllFilmsString.jsp");
	    dispatcher.forward(request, response);
	}
	if ("table".equals(format)) {
		response.setContentType("text/javascript");
		RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/JSP/AllFilms/AllFilmsTable.jsp");
		dispatcher.forward(request, response);
	}
  }
}
